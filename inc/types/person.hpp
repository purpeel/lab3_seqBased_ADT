...
public:
    ...
    Person( const std::string& name, unsigned age ): _name{name}, _age{age} {}
...
inline Person doubleUp( const Person& person ) {
        return Person( person.getName(), person.getAge() * 2 );
    }

inline Person sign( const Person& person ) {
    std::string newName = "";
    bool isFirst = true;
    for ( auto c : person.getName() ) {
        if ( c == ' ' ) {
            isFirst = true;
            continue;
        } else if ( c == '.' ) {
    ...
    return Person( newName, person.getAge() );
}

inline bool isEven( const Person& person ) {
    return person.getAge() % 2 == 0;
}
